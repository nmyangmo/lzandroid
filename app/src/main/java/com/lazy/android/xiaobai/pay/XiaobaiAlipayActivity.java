...
public class XiaobaiAlipayActivity extends LZBaseActivity {
			...
			switch (msg.what) {
				case SDK_PAY_FLAG: {
					...
					if (TextUtils.equals(resultStatus, "9000")) {
						...
						Intent intent = new Intent(XiaobaiAlipayActivity.this,CommonWebViewActivity.class);
						...
						if (TextUtils.equals(resultStatus, "8000")) {
							Toast.makeText(XiaobaiAlipayActivity.this, "支付结果确认中", Toast.LENGTH_SHORT).show();
						}else{
							// 其他值就可以判断为支付失败，包括用户主动取消支付，或者系统返回的错误
							Toast.makeText(XiaobaiAlipayActivity.this, "支付失败", Toast.LENGTH_SHORT).show();
						}
					}
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		new XiaobaiAlipayProtocol(this,this,orderid);
...
//		Toast.makeText(AlipayActivity.this, "payInfo=" + payInfo, Toast.LENGTH_SHORT).show();
		...
		Runnable payRunnable = new Runnable() {
			...
			public void run() {
				// 构造PayTask 对象
				PayTask alipay = new PayTask(XiaobaiAlipayActivity.this);
				// 调用支付接口，获取支付结果
	...
	public void onHttpSuccess(LZHttpProtocolHandlerBase protocol) {
		...
		if(protocol.getProtocolType() == ConfigProtocolType.PAY_ALIPAY){
				XiaobaiAlipayProtocol alipayProtocol = (XiaobaiAlipayProtocol) protocol;
