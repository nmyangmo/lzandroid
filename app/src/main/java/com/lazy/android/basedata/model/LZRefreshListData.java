...
public class LZRefreshListData<T extends LZBaseData> {
	...
	private int mPageSize = ConfigSystem.PAGE_SIZE;
	private int mType = TYPE_GET_NEXT_PAGE;
	private int mPageNum = START_INDEX;

	public LZRefreshListData() {
		mDataList = new ArrayList<T>();
		mDataMap = new TreeMap<Long, T>();
	}

	public LZRefreshListData(ArrayList<T> dataList) {
		this(dataList, new TreeMap<Long, T>());
	}

	public LZRefreshListData(TreeMap<Long, T> treeMap) {
		this(null, treeMap);
	}

	public LZRefreshListData(ArrayList<T> dataList, TreeMap<Long, T> treeMap) {
		mDataMap = treeMap;
		if (dataList != null) {
			mDataList = dataList;
			for (T data : dataList) {
	...
	public LZRefreshListData(ArrayList<T> dataList, BaseDataController<T> controller) {
		this(dataList);
		mDataDbController = controller;
	}

	public LZRefreshListData(TreeMap<Long, T> treeMap, BaseDataController<T> controller) {
		this(treeMap);
		if (controller != null) {
			mDataDbController = controller;
